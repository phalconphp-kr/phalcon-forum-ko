			...
			if ($user->email && $user->notifications != 'N' && strpos($user->email, '@users.noreply.github.com') === false) {
				...
				if (trim($escapedContent)) {
					...
					if ($notification->type == 'P') {
						...
						PHP_EOL . '<a href="http://forum.phalconphp.kr/discussion/' . $post->id. '/' . $post->slug . '">Phosphorum</a>. ';
					} else {
						$htmlContent .= '&mdash;<br>Reply to this email directly or view the complete thread on '.
						PHP_EOL . '<a href="http://forum.phalconphp.kr/discussion/' . $post->id. '/' . $post->slug . '#C' . $reply->id . '">Phosphorum</a>. ';
					}
					$htmlContent .= PHP_EOL . 'Change your e-mail preferences <a href="http://forum.phalconphp.kr/settings">here</a></p>';
